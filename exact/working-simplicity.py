...
complexity_base = {'+':-1,'-':-1,'*':-1,'esqr':0,'ecube':0}
...
class Block(object):
  def __init__(self,standardize,vars,emul,tmp='v'):
    self.vars = vars
  ...
  def compute(self,name,e,predefined=False):
    ...
    def exp(e): # (precedence,string)
      ...
      if op=='*' and self.emul and not args[0].is_Number:
        return 10,'%s(%s)'%(self.emul,','.join(exp(a)[1] for a in args))
      if len(args)==1:
        assert op=='-'
...
class Compiler(object):
  def __init__(self,interval_filter):
    self.header_lines = []
    self.source_lines = []
    self.forwards = []
    self.bodies = []
  ...
  def compile(self,predicate,d):
    ...
    with scope('predicate %s'%name):
      ...
      with scope('setup'):
        ...
        for line in predicate.__doc__.split('\n'):
          if line:
            line = '// '+line.strip()
            self.header(line)
        ET = 'exact::Real' if self.interval_filter else 'exact::Int'
        parameters = ', '.join('const int %si, const Vector<%s,%d> %s'%(a,ET,d,a) for a in args)
      ...
      # Prepare for nondegenerate evaluation
      ...
      if self.interval_filter:
        with scope('interval'):
          ...
          body.append('  {')
          block = Block(standardize,dict((v,'Interval(%s)'%s) for v,s in C),emul=None)
      ...
      with scope('constant'):
        if self.interval_filter:
          ...
          body.append('  // Evaluate constant term with exact integer arithmetic')
        body.append('  OTHER_UNUSED const exact::Int %s;'%', '.join('%s(%s)'%(v,s) for v,s in C))
        body.append('  assert(%s);'%' && '.join('%s==%s'%(v,s) for v,s in C))
        block = Block(standardize,dict((v,str(v)) for v,_ in C),emul='emul')
...
def incircle(p0,p1,p2,p3):
  '''Does p3 lie inside the circle defined by p0,p1,p2?  This predicate is antisymmetric.'''
  def row(p):
    d = p-p3
    return numpy.hstack([dot(d,d),d]) # Put the quadratic entry first so that subexpressions are lower order
  return det(row(p0),row(p1),row(p2))

### Top level

if __name__=='__main__':
  compiler = Compiler(interval_filter=False)
  ...
  compiler.compile(segment_to_direction_oriented,2)
  compiler.compile(directions_oriented,2)
  ...
  open('predicates-generated.h','w').write('\n'.join(compiler.header_lines)+'\n')
  open('predicates-generated.cpp','w').write('\n'.join(compiler.source_lines)+'\n')
