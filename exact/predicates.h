...
template<int axis,int d> OTHER_CORE_EXPORT OTHER_CONST bool axis_less_degenerate(const Tuple<int,Vector<exact::Int,d>> a, const Tuple<int,Vector<exact::Int,d>> b) OTHER_COLD;

// Is a[axis] < b[axis]?
template<int axis,int d> OTHER_CONST static inline bool axis_less(const Tuple<int,Vector<exact::Int,d>> a, const Tuple<int,Vector<exact::Int,d>> b) {
...
// Is a.x < b.x?
static inline bool rightwards(const Tuple<int,Vector<exact::Int,2>> a, const Tuple<int,Vector<exact::Int,2>> b) {
  if (a.y.x != b.y.x)
    return a.y.x < b.y.x;
...
// Is a.y < b.y?
static inline bool upwards(const Tuple<int,Vector<exact::Int,2>> a, const Tuple<int,Vector<exact::Int,2>> b) {
  if (a.y.y != b.y.y)
    return a.y.y < b.y.y;
