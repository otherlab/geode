...
namespace {
template<bool add> struct Intersect { template<class TV> static inline PredicateType<2,TV> eval(const TV S0, const TV S1) {
...
namespace {
struct UpwardsA { template<class TV> static PredicateType<5,TV> eval(const TV S0, const TV S1, const TV S2, const TV S3) {
  ...
  return ((c2.y-c0.y)<<1)*sqr_c01*sqr_c23+alpha23*(c23.y*sqr_c01)-alpha01*(c01.y*sqr_c23);
}};
