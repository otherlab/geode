...
typedef real T;
OTHER_DEFINE_TYPE(SparseMatrix)
...
multiply_helper(RawArray<const TV> x,RawArray<TV> result) const
{
    ...
    OTHER_ASSERT(columns()<=x.size() && rows<=result.size());
...
multiply_python(NdArray<const T> x,NdArray<T> result) const {
  OTHER_ASSERT(x.shape==result.shape);
  if(x.rank()==1)
...
{
    OTHER_ASSERT(cholesky && rows()<=x.size() && rows()<=b.size());
...
{
    OTHER_ASSERT(rows()==columns() && x.size()==rows() && b.size()==rows());
...
void wrap_sparse_matrix()
{
    ...
    Class<Self>("SparseMatrix")
        ...
        .OTHER_FIELD(J)
        .OTHER_FIELD(A)
        .OTHER_METHOD_2("multiply",multiply_python)
        .OTHER_METHOD(solve_forward_substitution)
        .OTHER_METHOD(solve_backward_substitution)
        .OTHER_METHOD(incomplete_cholesky_factorization)
        .OTHER_METHOD(gauss_seidel_solve)
        ;
}
