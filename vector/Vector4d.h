...
public:
  ...
  Vector(const T& x,const T& y,const T& z,const T& w)
    ...
    explicit Vector(const TVector& v, typename boost::enable_if<boost::mpl::and_<boost::is_same<T,typename TVector::Element>,mpl::bool_<TVector::m==4> >,Unusable>::type=Unusable())
    ...
    template<class TVector> typename boost::enable_if<boost::mpl::and_<boost::is_same<T,typename TVector::Element>,mpl::bool_<TVector::m==4> >,Vector&>::type
  ...
  {T mag=magnitude();if(mag) return *this*(1/mag);else return Vector(1,0,0,0);}
    ...
    T min() const
  {return other::min(x,y,z,w);}

    T max() const
  {return other::max(x,y,z,w);}

    T maxabs() const
  {return other::maxabs(x,y,z,w);}

    int argmin() const
  {return other::argmin(x,y,z,w);}

    int argmax() const
  {return other::argmax(x,y,z,w);}
  ...
  {return x<v.x && y<v.y && z<v.z && w<v.w;}
    ...
    {return Vector(other::max(v1.x,v2.x),other::max(v1.y,v2.y),other::max(v1.z,v2.z),other::max(v1.w,v2.zw));}
