...
public:
    ...
    T maxabs() const
    {return other::maxabs(x[0][0],x[1][0],x[0][1],x[1][1]);}
    ...
    void indefinite_polar_decomposition(Matrix& Q,SymmetricMatrix<T,2>& S) const OTHER_CORE_EXPORT;
    void fast_singular_value_decomposition(Matrix& U,DiagonalMatrix<T,2>& singular_values,Matrix& V) const OTHER_CORE_EXPORT;
};
