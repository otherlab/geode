...
template<class TV> static Frame<TV> rigid_register_helper(RawArray<const TV> X0, RawArray<const TV> X1, RawArray<const T> mass) {
  ...
  const int n = X0.size();
  OTHER_ASSERT(n==X1.size() && (mass.size()==0 || mass.size()==n));
...
void wrap_register() {
#ifdef OTHER_PYTHON
  OTHER_FUNCTION_2(rigid_register,rigid_register_python)
  OTHER_FUNCTION_2(affine_register,affine_register_python)
#endif
}
