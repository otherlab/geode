...
PolygonMesh(Array<const int> counts,Array<const int> vertices)
    :counts(counts),vertices(vertices)
{
    // assert validity and compute counts
    half_edge_count=counts.sum();
    OTHER_ASSERT(half_edge_count==vertices.size());
    for(int i=0;i<counts.size();i++) OTHER_ASSERT(counts[i]>=3); // polygons must be at least triangles
    node_count=0;
    ...
    if(!triangle_mesh_){
        ...
        for(int p=0;p<counts.size();p++){
            for(int i=0;i<counts[p]-2;i++)
                triangles[t++]=vec(vertices[offset],vertices[offset+i+1],vertices[offset+i+2]);
            offset+=counts[p];}
