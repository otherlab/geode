#include <other/core/mesh/HalfedgeGraph.h>
#include <other/core/python/Class.h>

namespace other {

OTHER_DEFINE_TYPE(HalfedgeGraph)
...
OTHER_CORE_EXPORT Ref<HalfedgeGraph> HalfedgeGraph::copy() const { return new_<HalfedgeGraph>(*this); }
...
OTHER_CORE_EXPORT EdgeId HalfedgeGraph::split_edge(const EdgeId e, const VertexId use_vertex) {
  ...
  OTHER_ASSERT(v_to_dst == halfedges_.append(HalfedgeInfo({src_to_v, next(src_to_dst), use_vertex})));
  OTHER_ASSERT(dst_to_v == halfedges_.append(HalfedgeInfo({prev(dst_to_src), v_to_src, orig_dst})));
...
OTHER_CORE_EXPORT EdgeId HalfedgeGraph::split_edge_across(const EdgeId e0, const HalfedgeId split) {
  ...
  // Add new halfedges
  OTHER_ASSERT(srcs_to_dst0 == halfedges_.append(HalfedgeInfo({dsts_to_srcs, next(src0_to_dst0), srcs})));
  OTHER_ASSERT(dst0_to_srcs == halfedges_.append(HalfedgeInfo({prev(dst0_to_src0), srcs_to_sn, dst0 })));
