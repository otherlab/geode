...
TriangleMesh::
TriangleMesh(Array<const Vector<int,3> > elements)
    :vertices(scalar_view_own(elements)),elements(elements),bending_quadruples_valid(false)
{
...
Ref<const SegmentMesh> TriangleMesh::
segment_mesh() const
{
    if(!segment_mesh_){
...
NestedArray<const int> TriangleMesh::
...
Array<const Vector<int,3> > TriangleMesh::
...
boundary_mesh() const
{
    if(!boundary_mesh_){
        ...
        for(int t=0;t<elements.size();t++)
            for(int i=0;i<3;i++){
                int a=elements[t][i],b=elements[t][(i+1)%3];
...
Array<const Vector<int,4> > TriangleMesh::
bending_quadruples() const
{
...
T TriangleMesh::
area(RawArray<const TV2> X) const {
...
T TriangleMesh::
volume(RawArray<const TV3> X) const
{
    OTHER_ASSERT(X.size()>=nodes());
...
T TriangleMesh::
surface_area(RawArray<const TV3> X) const
{
    OTHER_ASSERT(X.size()>=nodes());
    T sum=0;
    for(int t=0;t<elements.size();t++){
...
Array<T> TriangleMesh::
vertex_areas(RawArray<const TV3> X) const
{
    OTHER_ASSERT(X.size()>=nodes());
    ...
    for(int t=0;t<elements.size();t++){
        ...
        areas[i]+=area;areas[j]+=area;areas[k]+=area;}
    return areas;
}

Array<TV3> TriangleMesh::
vertex_normals(RawArray<const TV3> X) const {
  OTHER_ASSERT(X.size()>=nodes());
...
Array<TV3> TriangleMesh::
element_normals(RawArray<const TV3> X) const
{
    OTHER_ASSERT(X.size()>=nodes());
