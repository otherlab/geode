...
OTHER_DEFINE_TYPE(TriangleTopology)
...
void TriangleTopology::assert_consistent() const {
  ...
  for (const auto e : halfedges()) {
    ...
    OTHER_ASSERT(valid(p));
    OTHER_ASSERT(valid(n));
    OTHER_ASSERT(valid(r));
    ...
    if (f.valid())
      ...
      OTHER_ASSERT(is_boundary(e));
      OTHER_ASSERT(is_boundary(p));
      OTHER_ASSERT(is_boundary(n));
      OTHER_ASSERT(!is_boundary(r));
  ...
  OTHER_ASSERT(n_boundary_edges()+actual_erased==boundaries_.size());
}
...
Tuple<Array<int>, Array<int>, Array<int>> TriangleTopology::collect_garbage() {
  ...
  OTHER_ASSERT(vertex_to_edge_.size() == n_vertices_);
  ...
  OTHER_ASSERT(faces_.size() == n_faces_);
  ...
  OTHER_ASSERT(boundaries_.size() == n_boundary_edges_);
...
static void corner_mesh_destruction_test(TriangleTopology& mesh, const uint128_t key) {
  ...
  while (mesh2->n_vertices()) {
    OTHER_ASSERT(mesh2->n_faces()==mesh2->faces_.size());
  ...
  while (mesh.n_vertices()) {
    ...
    if (mesh.n_faces() && choice) {
      ...
      while (target.i == target.end) {
        OTHER_ASSERT(target.i != VertexId(0));
        imax = target.i.id;
