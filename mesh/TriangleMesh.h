...
protected:
  TriangleMesh(Array<const Vector<int,3> > elements) OTHER_CORE_EXPORT;
public:
  ~TriangleMesh();
  ...
  Ref<const SegmentMesh> segment_mesh() const OTHER_CORE_EXPORT;
  ...
  Ref<SegmentMesh> boundary_mesh() const OTHER_CORE_EXPORT;
  Array<const Vector<int,4> > bending_quadruples() const OTHER_CORE_EXPORT;
  Array<const int> nodes_touched() const OTHER_CORE_EXPORT;
  NestedArray<const int> sorted_neighbors() const OTHER_CORE_EXPORT; // vertices to sorted one-ring
  T area(RawArray<const TV2> X) const OTHER_CORE_EXPORT;
  T volume(RawArray<const TV3> X) const OTHER_CORE_EXPORT; // assumes a closed surface
  T surface_area(RawArray<const TV3> X) const OTHER_CORE_EXPORT;
  Array<T> vertex_areas(RawArray<const TV3> X) const OTHER_CORE_EXPORT;
  Array<TV3> vertex_normals(RawArray<const TV3> X) const OTHER_CORE_EXPORT;
  Array<TV3> element_normals(RawArray<const TV3> X) const OTHER_CORE_EXPORT;
