...
public:
  ...
  template<int k> RawArray<typename mpl::if_c<is_const,const Vector<Element,k>,Vector<T,k> >::type> vector_view() const {
    OTHER_ASSERT(n==k);typedef typename mpl::if_c<is_const,const Vector<Element,k>,Vector<T,k> >::type TV;
  ...
  OTHER_CORE_EXPORT void permute_rows(RawArray<const int> p,int direction) const; // 1 for forward (A[i] = A[p[i]]), -1 for backward (A[p[i]] = A[i])
  OTHER_CORE_EXPORT void permute_columns(RawArray<const int> p,int direction) const; // 1 for forward (A[i] = A[p[i]]), -1 for backward (A[p[i]] = A[i])
};

OTHER_CORE_EXPORT real frobenius_norm(RawArray<const real,2> A);
OTHER_CORE_EXPORT real infinity_norm(RawArray<const real,2> A); // Matrix infinity norm
...
struct TemporaryOwner {
  Ref<PyObject> owner;
  OTHER_CORE_EXPORT TemporaryOwner(); // creates owner with reference count 1
  OTHER_CORE_EXPORT ~TemporaryOwner(); // bails if reference count isn't back to 1
