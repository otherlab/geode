...
template<int d> static void segment_tests(const int steps) {
  ...
  for (int step=0;step<steps;step++) {
    ...
    OTHER_ASSERT(w1==clamp(w0,0.,1.));
    const auto close = segment_closest_point(s0,p);
    const auto c = close.x;
    OTHER_ASSERT(abs(w1-close.y)<tol);
    OTHER_ASSERT(magnitude(c-s0.interpolate(w1))<tol);
    const auto pd = segment_point_distance(s0,p);
    OTHER_ASSERT(abs(pd-magnitude(p-c))<tol);
    const auto r = segment_point_distance_and_normal(s0,p);
    OTHER_ASSERT(abs(pd-r.x)<tol);
    OTHER_ASSERT(abs(magnitude(r.y)-1)<tol);
    OTHER_ASSERT(magnitude(c-p+pd*r.y)<tol);
    OTHER_ASSERT(segment_point_distance(s0,c)<tol);
    OTHER_ASSERT(abs(segment_point_distance(s0,c+3.*r.y)-3)<tol);

    // Line / point
    const T ld = line_point_distance(s0,p);
    OTHER_ASSERT(ld <= pd+tol);
    const auto lclose = line_closest_point(s0,p);
    OTHER_ASSERT(abs(ld-magnitude(p-lclose.x))<tol);
    const auto lr = line_point_distance_and_normal(s0_,TV3(p));
    OTHER_ASSERT(abs(lr.x-ld)*len0<tol);
    OTHER_ASSERT(abs(magnitude(lr.y)-1)<tol);
    OTHER_ASSERT(abs(lr.z-lclose.y)*len0<tol);
    OTHER_ASSERT(magnitude(lclose.x-s0.interpolate(lr.z))<tol);

    // Segment / segment
    const T sd = segment_segment_distance(s0,s1);
    const auto sr = segment_segment_distance_and_normal(s0,s1);
    OTHER_ASSERT(abs(sr.x-sd)<tol);
    const auto n = sr.y;
    OTHER_ASSERT(abs(magnitude(n)-1)<tol);
    const auto w = sr.z;
    OTHER_ASSERT(0<=w.min() && w.max()<=1);
    const auto u = Vector<T,3>(s1.interpolate(w.y)-s0.interpolate(w.x));
    OTHER_ASSERT(abs(sd-magnitude(u))<tol);
    OTHER_ASSERT(magnitude(u-sd*n)<tol);
    const T d0 = dot(n,TV3(s0.vector()));
    // TODO: Make stronger assertions unconditional once the code is fixed
    // See https://github.com/otherlab/core/issues/42.
    if (sd>.1) {
      if (w.x>0) OTHER_ASSERT(d0>-tol);
      if (w.x<1) OTHER_ASSERT(d0<tol);
    }
    if (0<w.x && w.x<1) OTHER_ASSERT(abs(d0)<tol);
    const T d1 = -dot(n,TV3(s1.vector()));
    // TODO: Make stronger assertions unconditional once the code is fixed
    // See https://github.com/otherlab/core/issues/42.
    if (sd>.1) {
      if (w.y>0) OTHER_ASSERT(d1>-tol);
      if (w.y<1) OTHER_ASSERT(d1<tol);
    }
    if (0<w.y && w.y<1) OTHER_ASSERT(abs(d1)<tol);

    // Line / line
    if (d==3) {
      const auto r = line_line_distance_and_normal(s0_,s1_);
      OTHER_ASSERT(r.x <= sd+tol);
      const auto n = r.y;
      const auto w = r.z;
      const auto u = s1_.interpolate(w.y)-s0_.interpolate(w.x);
      OTHER_ASSERT(abs(r.x-magnitude(u))*min_len<tol);
      OTHER_ASSERT(magnitude(u-r.x*n)*min_len<tol);
      OTHER_ASSERT(abs(dot(n,s0_.vector()))<tol);
      OTHER_ASSERT(abs(dot(n,s1_.vector()))<tol);
    }
  }
}
...
void wrap_segment() {
  OTHER_FUNCTION_2(segment_tests_2d,segment_tests<2>)
  OTHER_FUNCTION_2(segment_tests_3d,segment_tests<3>)
}
