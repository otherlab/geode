...
public:
    ...
    {return other::bounding_box(X[0],X[1],X[2]);}
    ...
    TV closest_point(const TV& location,Vector<T,3>& weights) const OTHER_CORE_EXPORT;
    TV closest_point(const TV& location) const OTHER_CORE_EXPORT;
    T distance(const TV& location) const OTHER_CORE_EXPORT;
};
