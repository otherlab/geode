...
protected:
  OTHER_EXPORT Bezier();
  OTHER_EXPORT Bezier(const Bezier<d>& b);
public:
  map<real,Ref<Knot<d>>> knots;
  OTHER_EXPORT vector<TV> segment(const InvertableBox& range, int res) const ;
  OTHER_EXPORT vector<TV> alen_segment(const InvertableBox& range, int res) const ;
  OTHER_EXPORT vector<TV> evaluate(int res) const ;
  OTHER_EXPORT vector<TV> alen_evaluate(int res) const ;
  OTHER_EXPORT void append_knot(const TV& pt, TV tin = T(inf)*TV::ones(), TV tout = T(inf)*TV::ones());
  OTHER_EXPORT void insert_knot(const real t) ;
  OTHER_EXPORT Span<d> segment(real t) const ;
  OTHER_EXPORT TV point(real t) const ;
  OTHER_EXPORT TV tangent(real t) const ;
  inline real t_max() const { return t_range.max; }
  inline real t_min() const { return t_range.min; }
  OTHER_EXPORT void close() ;
  OTHER_EXPORT void fuse_ends() ;
  OTHER_EXPORT void erase(real t) ;
  inline bool closed() const { return b_closed; }
  OTHER_EXPORT ~Bezier();

  OTHER_EXPORT Box<TV> bounding_box() const;

  OTHER_EXPORT void translate(const TV& t);
...
template<int d> struct PointSolve {
  ...
  OTHER_EXPORT real distance(real t);
};
