...
template<class T> PyObject* to_python(const Box<T>& self) OTHER_EXPORT;
template<class T> struct FromPython<Box<T> >{OTHER_EXPORT static Box<T> convert(PyObject* object);};
