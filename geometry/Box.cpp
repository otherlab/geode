...
template<class TV> PyObject* to_python(const Box<TV>& box)
{
    try{return to_python(new_<AnalyticImplicit<Box<TV> > >(box));}
    catch(std::exception& error){set_python_exception(error);return 0;}
}

template<class TV> Box<TV> FromPython<Box<TV> >::
convert(PyObject* object)
{
    return from_python<AnalyticImplicit<Box<TV> >&>(object);
}   
...
surface(const TV& X) const
{
    ...
    TV min_side=X-min,max_side=max-X,result=X;
    int min_argmin=min_side.argmin(),max_argmin=max_side.argmin();
    if(min_side[min_argmin]<=max_side[max_argmin]) result[min_argmin]=min[min_argmin];
    else result[max_argmin]=max[max_argmin];
    return result;
}
...
{
    if(lazy_inside(X)){
        TV phis_max=X-max,phis_min=min-X;
...
template<class TV> string Box<TV>::
name()
{
...
template<class TV> bool Box<TV>::
lazy_intersects(const Ray<TV>& ray,T box_enlargement) const
{
...
// This is a fast routine to do ray box intersections
// box_enlargement modifies the bounds of the box -- it's not a thickness
template<> bool Box<Vector<T,2> >::
lazy_intersects(const Ray<Vector<T,2> >& ray,T box_enlargement) const
{
...
// This is a fast routine to do ray box intersections
// box_enlargement modifies the bounds of the box -- it's not a thickness
template<> bool Box<Vector<T,3> >::
lazy_intersects(const Ray<Vector<T,3> >& ray,T box_enlargement) const
{
...
#define INSTANTIATION_HELPER(T,d) \
    ...
    template PyObject* to_python<Vector<T,d> >(const Box<Vector<T,d> >&); \
