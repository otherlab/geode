...
void copy_to_file(const string& filename,const bool append) {
  ...
  if (!filename.empty()) {
    if (append) {
      ...
      if (!log_file) OTHER_FATAL_ERROR(format("Can't open log file %s for append",filename));
      putc('\n',log_file);
    } else {
      log_file = fopen(filename.c_str(),"w");
      if(!log_file) OTHER_FATAL_ERROR(format("Can't open log file %s for writing",filename));
    }
    if (temporary_file) {
