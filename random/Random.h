...
protected:
  explicit Random(uint128_t seed) OTHER_CORE_EXPORT;
public:
  ~Random();

  template<class Int> Int bits() OTHER_CORE_EXPORT;
  ...
  template<class TV> Rotation<TV> rotation() OTHER_CORE_EXPORT;
  template<class TV> Frame<TV> frame(const TV& v0,const TV& v1) OTHER_CORE_EXPORT;
};

// In [a,b)
template<>   int8_t Random::uniform(const   int8_t a, const   int8_t b) OTHER_CORE_EXPORT;
template<>  uint8_t Random::uniform(const  uint8_t a, const  uint8_t b) OTHER_CORE_EXPORT;
template<>  int16_t Random::uniform(const  int16_t a, const  int16_t b) OTHER_CORE_EXPORT;
template<> uint16_t Random::uniform(const uint16_t a, const uint16_t b) OTHER_CORE_EXPORT;
template<>  int32_t Random::uniform(const  int32_t a, const  int32_t b) OTHER_CORE_EXPORT;
template<> uint32_t Random::uniform(const uint32_t a, const uint32_t b) OTHER_CORE_EXPORT;
template<>  int64_t Random::uniform(const  int64_t a, const  int64_t b) OTHER_CORE_EXPORT;
template<> uint64_t Random::uniform(const uint64_t a, const uint64_t b) OTHER_CORE_EXPORT;
