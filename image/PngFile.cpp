...
write_helper(const std::string& filename,RawArray<const Vector<T,C>,2> image)
{
    ...
    if(!file) OTHER_FATAL_ERROR(format("Failed to open %s for writing",filename));

    png_structp png_ptr=png_create_write_struct(PNG_LIBPNG_VER_STRING,0,0,0);
    if(!png_ptr) OTHER_FATAL_ERROR(format("Error writing png file %s",filename));
    png_infop info_ptr=png_create_info_struct(png_ptr);
    if(!info_ptr) OTHER_FATAL_ERROR(format("Error writing png file %s",filename));
    if(setjmp(png_jmpbuf(png_ptr))) OTHER_FATAL_ERROR(format("Error writing png file %s",filename));
    png_init_io(png_ptr,file);
...
template<class T,int C> std::vector<unsigned char> write_to_memory_helper(RawArray<const Vector<T,C>,2> image)
{
  ...
  png_structp png_ptr=png_create_write_struct(PNG_LIBPNG_VER_STRING,0,0,0);
  if(!png_ptr) OTHER_FATAL_ERROR("Error writing png file to buffer");
