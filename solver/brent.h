...
Tuple<real,real,int> brent(const function<real(real)>& f, Vector<real,2> bracket, real xtol, int maxiter) OTHER_CORE_EXPORT;

// Same as above, but specify the full bracketing interval explicitly.  bracket = (a,b,c) must satisfy b in [a,c], f(b) < f(a),f(c).
Tuple<real,real,int> brent(const function<real(real)>& f, Vector<real,3> bracket, real xtol, int maxiter) OTHER_CORE_EXPORT;
