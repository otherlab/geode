...
write(const std::string& filename,RawArray<const Vector<T,3>,2> image)
{
    ...
    if(!(outfile = fopen(filename.c_str(), "wb")))
        GEODE_FATAL_ERROR(format("JpgFile::write: Can't open %s",filename));
...
read(const std::string& filename)
{
    GEODE_FATAL_ERROR("Not compiled with GEODE_LIBJPEG.  Cannot read jpeg image.");
}
//#####################################################################
// Function Write
//#####################################################################
...
write(const std::string& filename,RawArray<const Vector<T,3>,2> image)
{
    GEODE_FATAL_ERROR("Not compiled with GEODE_LIBJPEG.  Cannot write jpeg image.");
}
//#####################################################################
// Function is_supported
//#####################################################################
