...
public:
    ...
    T maxabs() const
    {return geode::maxabs(x00,x10,x20,x11,x21,x22);}
    ...
    {T scale=geode::maxabs(eigenvalues.x00,eigenvalues.x22),scale_inverse=robust_inverse(scale),tiny=tolerance*scale;
...
//#####################################################################
    ...
    GEODE_CORE_EXPORT DiagonalMatrix<T,3> fast_eigenvalues() const ;
    GEODE_CORE_EXPORT void fast_solve_eigenproblem(DiagonalMatrix<T,3>& eigenvalues,Matrix<T,3>& eigenvectors) const ;
