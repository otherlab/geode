...
public:
  ...
  template<class T2,int d2> explicit Vector(const Vector<T2,d2>& v) {
    static_assert(d2<=d,"");
    for(int i=0;i<d2;i++) array[i]=T(v[i]);
    for(int i=d2;i<d;i++) array[i]=T();
  }

  template<class TVector> explicit Vector(const TVector& v) {
  ...
  template<int n> Vector(const Vector<T,n>& v1,const Vector<T,d-n>& v2) {
    for(int i=0;i<n;i++) (*this)(i)=v1(i);for(int i=n;i<d;i++) (*this)(i)=v2(i-n);
  }

  template<class TVector> typename EnableForVectorLike<T,d,TVector,Vector&>::type operator=(const TVector& v) {
