...
public:
    ...
    T maxabs() const
    {return geode::maxabs(x[0][0],x[1][0],x[2][0],x[0][1],x[1][1],x[2][1]);}
...
//#####################################################################
    GEODE_CORE_EXPORT void fast_singular_value_decomposition(Matrix& U,DiagonalMatrix<T,2>& singular_values,Matrix<T,2>& V) const ;
    GEODE_CORE_EXPORT void fast_indefinite_polar_decomposition(Matrix<T,3,2>& Q,SymmetricMatrix<T,2>& S) const ;
//#####################################################################
};
