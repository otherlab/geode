...
public:
    ...
    Vector<T,3>& row(const int i)
    { return (*this)[i]; }

    const Vector<T,3>& row(const int i) const
    { return (*this)[i]; }
    ...
    T maxabs() const
    {return geode::maxabs(x[0][0],x[1][0],x[2][0],x[0][1],x[1][1],x[2][1],x[0][2],x[1][2],x[2][2]);}
...
//#####################################################################
    ...
    GEODE_CORE_EXPORT void fast_singular_value_decomposition(Matrix<T,3>& U,DiagonalMatrix<T,3>& singular_values,Matrix<T,3>& V) const ;
    GEODE_CORE_EXPORT void fast_indefinite_polar_decomposition(Matrix<T,3>& Q,SymmetricMatrix<T,3>& S) const ;
    GEODE_CORE_EXPORT T simplex_minimum_altitude() const ;
//#####################################################################
};
// global functions
template<class T>
