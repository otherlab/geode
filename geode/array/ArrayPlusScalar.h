...
class ArrayPlusScalar : public ArrayExpression<typename Sum<T1,typename TArray2::Element>::type,GEODE_IF_MSVC(typename) ArrayPlusScalar<T1,TArray2>,TArray2> {
...
template<class T1,class T2> struct ArrayPlusScalarValid<T1,T2,typename mpl::if_<mpl::true_,void,typename Sum<T1,T2>::type>::type>:public
  mpl::or_<is_same<typename remove_const<T1>::type,typename remove_const<T2>::type>,IsScalar<T1> >{};

template<class T1,class T2,class TArray2> static inline typename enable_if<ArrayPlusScalarValid<T1,T2>,ArrayPlusScalar<T1,TArray2> >::type
