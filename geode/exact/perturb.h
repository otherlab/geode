...
template<int m> GEODE_CORE_EXPORT GEODE_COLD bool perturbed_sign(void(*const predicate)(RawArray<mp_limb_t>,RawArray<const Vector<Exact<1>,m>>), const int degree, RawArray<const Tuple<int,Vector<Quantized,m>>> X);
...
template<int m> GEODE_CORE_EXPORT GEODE_COLD void perturbed_ratio(RawArray<Quantized> result, void(*const ratio)(RawArray<mp_limb_t,2>,RawArray<const Vector<Exact<1>,m>>),
...
template<class F,int d,class... entries> GEODE_ALWAYS_INLINE static inline auto wrap_predicate(Types<entries...>)
...
template<class A,class B,class C,class... Args> GEODE_ALWAYS_INLINE static inline bool perturbed_predicate_sqrt(const int sign, const Args... args) {
  assert(abs(sign)==1);
...
template<class A,class B0,class B1,class C0,class C1,class... Args> GEODE_ALWAYS_INLINE static inline bool perturbed_predicate_two_sqrts(const int sign0, const int sign1, const Args... args) {
