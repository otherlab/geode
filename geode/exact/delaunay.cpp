...
GEODE_ALWAYS_INLINE static inline bool is_delaunay(const CornerMesh& mesh, RawField<const Point,VertexId> X, const HalfedgeId edge) {
...
GEODE_UNUSED static void check_bsp(const CornerMesh& mesh, RawArray<const Node> bsp, RawField<const Vector<int,2>,FaceId> face_to_bsp, RawField<const Point,VertexId> X_) {
  if (self_check) {
    cout << "bsp:\n";
...
GEODE_COLD static void assert_delaunay(const CornerMesh& mesh, RawField<const Point,VertexId> X, const bool oriented_only=false) {
...
GEODE_NEVER_INLINE static Ref<CornerMesh> deterministic_exact_delaunay(RawField<const Point,VertexId> X, const bool validate) {
