...
template<int a,int b> GEODE_PURE static inline Exact<a+b> operator*(const Exact<a> x, const Exact<b> y) {
...
template<int a> GEODE_PURE static inline Exact<a> small_mul(const int n, const Exact<a> x) {
  assert(n);
  Exact<a> r(uninit);
