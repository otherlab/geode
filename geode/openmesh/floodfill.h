...
template<class EdgeStop> vector<VertexHandle> floodfill(const TriMesh& mesh, RawArray<const VertexHandle> seeds, const EdgeStop& stop) GEODE_NEVER_INLINE;
...
template<class EdgeStop> vector<TriMesh::FaceHandle> floodfill(const TriMesh& mesh, RawArray<const FaceHandle> seeds, const EdgeStop& stop) GEODE_NEVER_INLINE;
template<class EdgeStop> unordered_set<HalfedgeHandle,Hasher> floodfill_boundary(const TriMesh& mesh, FaceHandle seed, const EdgeStop& stop) GEODE_NEVER_INLINE;
template<class EdgeStop> Tuple<vector<FaceHandle>,unordered_set<HalfedgeHandle,Hasher> > floodfill_data(const TriMesh& mesh, FaceHandle seed, const EdgeStop& stop) GEODE_NEVER_INLINE;
