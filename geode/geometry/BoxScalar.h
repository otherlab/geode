...
// but arithmetic operations are *not* exact.  See geode/exact if you need
// exact operations.
//
//#####################################################################
#pragma once
...
public:
  ...
  T maxabs() const {
    return geode::maxabs(min,max);
  }

  void enlarge(const T& point) {
    min = geode::min(min,point);
    max = geode::max(max,point);
  }
  ...
  void enlarge(const Box& box) {
    min = geode::min(min,box.min);
    max = geode::max(max,box.max);
  }
  ...
  static Box combine(const Box& box1, const Box& box2) {
    return Box(geode::min(box1.min,box2.min),geode::max(box1.max,box2.max));
  }

  static Box intersect(const Box& box1,const Box& box2) {
    return Box(geode::max(box1.min,box2.min),geode::min(box1.max,box2.max));
  }
