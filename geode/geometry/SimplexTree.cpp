...
template<class TV,int d> void SimplexTree<TV,d>::update() {
  ...
  for (const int n : leaves) {
    ...
    for (const int s : prims(n))
      box.enlarge(geode::bounding_box(X.subset(elements[s])));
    boxes[n] = box;
  }
...
namespace {
template<class T> struct PlaneVisitor {
  ...
  bool cull(const int node) const {
    return !plane.template intersection<Zero>(self.boxes[node]);
  }
...
template<> void SimplexTree<Vector<real,3>,2>::intersections(const Plane<real>& plane, Array<Segment<Vector<real,3>>>& results) const {
  results.clear();
...
template<int signs,class TV,int d> static void intersection_helper(const SimplexTree<TV,d>& self, Ray<TV>& ray, const typename TV::Scalar half_thickness) {
  ...
  RawStack<Tuple<int,T>> stack(GEODE_RAW_ALLOCA(self.depth,Tuple<int,T>)); // Each entry is (node,t_min)
...
template<class T> static inline bool going_out(const Segment<Vector<T,3>>& s, const Vector<T,3>& d) { GEODE_NOT_IMPLEMENTED(); }
template<class T> static inline bool going_out(const Triangle<Vector<T,2>>& s, const Vector<T,2>& d) { GEODE_NOT_IMPLEMENTED(); }

template<class Simplex,class TV> static inline bool inside_plane(const Simplex& s, const TV& p) { return going_out(s,s.x0-p); }
template<class T> static inline bool inside_plane(const Triangle<Vector<T,2>>& s, const Vector<T,2>& p) { GEODE_NOT_IMPLEMENTED(); }
...
inside_given_closest_point(TV point, int simplex, Weights weights) const {
  GEODE_ASSERT(mesh->elements.valid(simplex));
  const T small = sqrt(numeric_limits<T>::epsilon());
