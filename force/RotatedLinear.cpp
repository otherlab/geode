...
class RotatedLinear:public IsotropicConstitutiveModel<T,d>
{
...
public:
    ...
    if(!mu.rank()) return mu()*strain.sqr_frobenius_norm()+(T).5*lambda()*sqr(strain.trace());
    else return mu[simplex]*strain.sqr_frobenius_norm()+(T).5*lambda[simplex]*sqr(strain.trace());}

    DiagonalMatrix<T,d> P_From_Strain(const DiagonalMatrix<T,d>& F,const T scale,const int simplex) const
    {DiagonalMatrix<T,d> strain = F-1;
    if(!mu.rank()) return 2*scale*mu()*strain+scale*lambda()*strain.trace();
