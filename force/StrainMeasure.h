...
template<class TV,int d> class StrainMeasure : public Object {
...
public:
  ...
  template<class TArray> static TMatrix Ds(const TArray& X,const Vector<int,2>& nodes) {
    int i,j;nodes.get(i,j);
    return TMatrix(X(j)-X(i));
  }

  template<class TArray> static TMatrix Ds(const TArray& X,const Vector<int,3>& nodes) {
    int i,j,k;nodes.get(i,j,k);
    return TMatrix(X(j)-X(i),X(k)-X(i));
  }

  template<class TArray> static TMatrix Ds(const TArray& X,const Vector<int,4>& nodes) {
