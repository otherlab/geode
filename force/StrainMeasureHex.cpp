...
OTHER_DEFINE_TYPE(StrainMeasureHex)
...
StrainMeasureHex(Array<const Vector<int,8> > elements,Array<const TV> X)
  :nodes(elements.size()?scalar_view(elements).max()+1:0),elements(elements),rest_X(X) {
  OTHER_ASSERT(nodes<=X.size());
  H_DmH_inverse.resize(elements.size(),false,false);
  DmH_det.resize(elements.size(),false,false);
  for(int h=0;h<elements.size();h++) for(int g=0;g<8;g++) {
    Matrix<T,3> DmH;for(int k=0;k<8;k++) DmH+=outer_product(X[elements[h][k]],Hg[g][k]);
    DmH_det[h][g] = DmH.determinant();
    OTHER_ASSERT(DmH_det[h][g]>0);
...
T StrainMeasureHex::
DmH_minimum_altitude(int hex) const {
  T result = FLT_MAX;
  for(int g=0;g<8;g++) {
    Matrix<T,3> DmH;for(int k=0;k<8;k++) DmH+=outer_product(rest_X[elements[hex][k]],Hg[g][k]);
    result = min(result,DmH.simplex_minimum_altitude());
  }
  return result;
}

Matrix<T,3> StrainMeasureHex::
gradient(RawArray<const TV> X,int hex,TV w) const {
  TV xi = (T)2*w-1;
  Matrix<T,8,3> H;
  for(int ii=0;ii<2;ii++) for(int jj=0;jj<2;jj++) for(int kk=0;kk<2;kk++) {
    TV s(2*ii-1,2*jj-1,2*kk-1),sxi=s*xi;
...
void wrap_strain_measure_hex() {
  ...
  Class<Self>("StrainMeasureHex")
    .OTHER_INIT(Array<const Vector<int,8> >,Array<const Vector<T,3> >)
